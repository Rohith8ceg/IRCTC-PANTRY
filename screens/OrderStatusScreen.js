    ...
    return (
      <ListItem
        ...
        accessoryRight={(props) => {
          ...
          return <Text>{getStatus(param.item.data.status)}</Text>;
        }}
      />
    );
  };
    ...
    <>
      {user.phone && (
        <Layout style={styles.layout}>
          <Text
            onPress={getData}
            style={{ fontSize: 26, fontWeight: "bold", marginBottom: 50 }}
          >
            {" "}
            {user.name}'s Order Status
          </Text>
          {orders.length > 0 && (
            <List
              style={{ flexGrow: 0, marginTop: 50, width: "80%" }}
